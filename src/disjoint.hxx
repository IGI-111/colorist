...
template <typename T> Disjoint<T>::Disjoint(const T &singleton) : length(1) {
  this->emplace_front(this, singleton);
}

template <typename T> Node<T> *Disjoint<T>::repr() { return &(this->front()); }

template <typename T> std::size_t Disjoint<T>::size() const { return length; }

template <typename T>
void Disjoint<T>::unite(Disjoint<T> *first, Disjoint<T> *second) {
