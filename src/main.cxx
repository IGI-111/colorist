    ...
    for (std::size_t y = 0; y < nodes.size(); ++y) {
        for (std::size_t x = 0; x < nodes[y].size(); ++x) {
            if (nodes.at(x, y)->content != Color::black) {
                ...
                here =
                    nodes.at(x, y)->parent;  // might have changed during union
                if (y + 1 < nodes.size() &&
...
int main(int argc, char *argv[]) {
    try {
        ...
        Matrix<Color> bitmap;
        if (argc == 3) {
            bitmap = pixmap::readMonochrome(argv[1]);
        } else {
            bitmap = pixmap::randomMonochrome(std::stoul(argv[1]),
                                              std::stoul(argv[2]));
        }
        ...
        pixmap::writeColored(bitmap, argv[argc - 1]);
